...
int main(int argc, char *argv[]){
    ...
    while ((c = getopt_long(argc,argv,opts,long_opts,NULL)) != -1){
        switch (c) {
            case 'h':{
                printf("--help -h  = print help\n--info -i [var_name]  = print variable\n--set -s [var_name=var] = sets or modifies an environment variable\n--assign -a [var_name] --value -v[var] = sets or modifies an environment variable\n--del -d[var] = delete environment variable\n--clear -c = clear environment variable\n");
            }break;
            case 'i':{
